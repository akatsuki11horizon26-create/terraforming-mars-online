...
内訳は **最終得点・セットアップ・特許売却** と、後述の到達不能コード。

**`app/page.tsx` の緑地/海洋/都市の配置分岐（1039-1092行付近）は到達不能。**
項目5で標準プロジェクトがコマンド層へ移ったあとも消し忘れられている。
現在 `sourceProject` に入る値は最終緑化の `"plants"` だけで、
`"greenery"` `"ocean"` `"city"` は誰も設定しない（`grep -n 'sourceProject:' app/page.tsx` で確認できる）。

削除は未実施。同じブロックの下半分（Mining Guild / Tharsis Republic の配置効果）は
カード配置からも通る**生きた経路**なので、巻き込まずに消す必要がある。
...
**ブラウザで一度も動作確認していない。**
テスト241件・CI・プレイテストは通っているが、実際の操作感は未確認。
...
- **オンラインの表示全般（`19f327d`）**。`activeState` 基準へ移したが、
  **これはテストで担保できていない**。テスト250件もプレイテストもオフライン経路しか通らない。
