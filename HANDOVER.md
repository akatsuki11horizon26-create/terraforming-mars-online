...
最終更新: 2026-08-19 / ブランチ `main` (`606faf9`) / テスト434件
...
| カットイン／行動パネル | ✅ | §6.5 |
| **オンライン対戦（標準プロジェクトのみ）** | ✅ | **本番で世代1〜11の完走を実測**（§6.8） |
| **オンラインでのカードプレイ・タイル配置・攻撃・拡張** | ❌ **未確認** | 下記 |
| ドラフトの実プレイ（UIから通しで） | ❌ 未確認 | エンジンは検証済み（§6.4） |

**オンラインの完走に使ったドライバは、カードを一枚も買わず標準プロジェクトだけで
終局させている。** カードプレイ・タイル配置の選択・攻撃カード・拡張（動乱／植民地／
金星／プレリュード）は**オンラインでは一度も通っていない**。
§6.8 の手法（WebSocket ドライバ）をそのまま使えば検証できる。
