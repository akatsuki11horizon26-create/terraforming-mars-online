...
最終更新: 2026-08-18 / ブランチ `main` / テスト431件
...
### 6.4 ドラフトはオンラインからしか有効にできない

`multiplayer-lobby.tsx:239` にチェックボックスがあるが、
**`page.tsx` に `draft` の記述が1件も無い**。つまりソロ・ロボット戦・ホットシートの
設定画面はドラフトを一切渡さない。エンジンは `playerCount > 1` なら動くので、
**ホットシートでも本来使えるはず**。意図的な制限なのか渡し忘れなのかは未確認。

**ホットシートの入口も分かりにくい。** タイトル画面は3択（ソロ/ロボット/オンライン）で、
人数選択は**ゲーム中ヘッダの「新規ゲーム設定」**にしか出ない（`intent === "custom"` のとき、
`title-screen.tsx:139`）。タイトルからホットシートには到達できない。
