...
最終更新: 2026-08-17 / ブランチ `fix-multiplayer-winner` / テスト424件
...
`MISSION SUCCESS` と出る状態が、**424件のテストとCI全緑のまま**残っていた。
...
特殊VP3枚のカード面バッジ以外の攻撃カードUI、5マップの盤面。
...
0. **最後まで実際に遊ぶ。** テストは「書かれた処理が正しいか」しか見ない。
   **書かれていない処理**（マルチの勝者判定）は、424件全緑・CI全緑・
   プレイテスト0件のまま3週間見逃されていた。終局画面まで到達して
