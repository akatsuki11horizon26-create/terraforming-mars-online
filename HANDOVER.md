...
最終更新: 2026-09-02 / ブランチ `main` / テスト662件 + E2E 5本

**§6.5 に上流差分ハーネスを追加**（2026-09-02）。上流のテストスイートを走らせて
記録し、こちらのエンジンで再生して照合する。実測464件が一致し、16件のバグを掘り出した。

**§6.10 にブラウザE2E（Playwright）を追加**（2026-09-02）。5本。
**立ち上げる過程で実バグを2件掘り出した**——どちらも
「エンジンは正しく計算し、DOMに出なかった」型で、662件のテストにも
監査23本にも一度も引っかかっていなかった。

作業ツリーはクリーン。`main` は `origin/main` と同一（`4850b35`）。
**両方の公開先に配信済み**（CIの緑ではなく、配信中のバンドルを直接検証した）。
...
- **§6.10 ブラウザE2E** — Playwright 5本。導入時に実バグ2件を掘り出した
- **§7** 開発の進め方
- **§8** 参考資料
...
| オンラインの通信量・切断復帰 | ❌ 未確認 | 再接続は実装あり（`use-room.ts`）だが未検証 |
...
### 未着手のシナリオ（外部レビューが出した5本のうち3本）

**致命度で切って、シナリオ3だけ先にやった。** 残りは以下。

| # | 名前 | 対象 | 検証すること | 壊し方 |
|---|---|---|---|---|
| 2 | タイル配置と盤面 | static | 帯水層18MC→合法セル→`.hex-ocean` 化・海洋 `1/9`・TR 14→15・カットイン | `tileChoiceCells` のマス選択ハンドラを無効化 |
| 4 | Pages配信の制約 | static | `/` を開くだけ。`mode-online` が `disabled`・`title="この配信版はソロ専用です"` | `title-screen.tsx` の `disabled={!onlineEnabled}` を削除 |
| 5 | オンライン2画面 | **Workers** | 部屋作成→合言葉参加→開始。両者で `pageerror` 0件・`corp-confirm-button` 可視 | `net-protocol.js` の `hydrateView()` を外す |

- **4が一番軽い**（ページを開くだけ）。2は1と導線を共有できる
- **5だけ別ビルド**。`npm run dev` が wrangler + Durable Objects を立てる必要があり、
  **CI（ubuntu-latest）での安定性は未検証**。全世代の進行はWSドライバの担当で、
  ここは接続と企業選択画面までのクラッシュ検知だけ
- **重複させないこと。** E2Eの守備範囲は「クリックを飲むモーダル・
  マウントしないボタン・DOMに出ない値」。数値網羅は監査、全世代同期はWSドライバ
...
```bash
npm test                 # ビルド + 662テスト
npm run test:e2e         # Playwright 5本（static-dist を配信して実ブラウザで叩く）
npm run lint             # ESLint
