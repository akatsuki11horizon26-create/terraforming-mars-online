...
日本語版ルール説明書（第3刷）の全文書き起こしと突き合わせて確認した数値。

| 項目 | 実装 | 出典 |
|---|---|---|
| 初期TR | 多人数20 / ソロ14 | P7, P13 |
| 初期MC | 42 | P7 |
| 気温 | -30℃ → +8℃ | P3 |
| 酸素濃度 | 14% | P3 |
| 海洋 | 9枚 | P3「９枚の海洋タイル」 |
| カード購入 | 1枚3 MC | P7「１枚につき３Ｍ€」 |
| 標準プロジェクト | 発電所11 / 小惑星14 / 帯水層18 / 緑地23 / 都市25 | P10 |
| マイルストーン | 8 MC | P11 |
| 表彰 | 8 / 14 / 20 MC | P11 |
| テラフォーマー | TR 35 | P11 |
| 盤面 | 61マス | P4 |
...
P8「手番では、１回もしくは２回のアクションを選択／実行できます。**１つもアクションを実行しなければ（パス）**、そのプレイヤーはいったん抜けることになり、その世代ではもはやアクションを実行できません。逆にアクションを１回でも実行する限り、手番は何度でも巡ってきます。全員がパスすると、アクション・フェイズは終了します。」

つまり:

- パス = そのターンに**1つもアクションを実行しないこと**。宣言した世代からは離脱する
- 1アクション実行後に手番を終えるのは**パスではなくターン終了**。次の手番はまた巡ってくる
- 全員がパスして初めてアクション・フェイズが終わる

`passPlayer` は既に行動済みなら `endTurn` に委譲する。UIのボタンも「パス (この世代を離脱)」と「ターン終了」で文言が変わる。
